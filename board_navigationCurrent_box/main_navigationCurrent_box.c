...
Uart_t *currnet_uart = 0;
unsigned char current_sensor_cmd[8]={0x01,0x03,0x00,0x48,0x00,0x08,0xc4,0x1a};
#define CURRENT_SENSOR_BUFFER_SIZE 37
unsigned char current_sensor_data[37]={0};
int current_sensor_index= 0;
systick_time_t current_time;

void current_sensor_init(Uart_t *uart)
{
	currnet_uart = uart;
	systick_time_start(&current_time,1000);
}
...
void current_sensor_loop()
{
	...
	if( currnet_uart == 0 ) return;
	
	if( Uart_GetChar(currnet_uart,&c) > 0 )
	{
		if( current_sensor_index >= CURRENT_SENSOR_BUFFER_SIZE ) 
			current_sensor_index = 0;
		current_sensor_data[current_sensor_index++] = c;
		
		//check head
		if( current_sensor_index == 2 ) 
		{
			if( current_sensor_data[0] != current_sensor_cmd[0] || current_sensor_data[1] != current_sensor_cmd[1] )
			{
				current_sensor_index = 0;
			}
		}
		//check len
		if( current_sensor_index == 3 )
		{
			i = ((current_sensor_cmd[4]<<8) | current_sensor_cmd[5]);
			i = i*4; //data_count * 4 = data len
			if( current_sensor_data[2] != i ){
				current_sensor_index = 0;
			}
			
		}
		
		i = current_sensor_data[2]*4 +3 +2; // 3:head tag; 2:crc; data_count * 4 = data len
		if( current_sensor_index == i )
		{
			//get a packget
			crc = (current_sensor_data[current_sensor_index-1]<<8 | current_sensor_data[current_sensor_index-2] );
			if( crc == crc_calculate(current_sensor_data,current_sensor_index-2) )
			{
				cmdcoder_send_bytes(&encoder, current_sensor_data , current_sensor_index);
			}
			current_sensor_index = 0;
		}
	}
	...
	if( check_systick_time(&current_time) )
	{
		for(i=0; i< sizeof(current_sensor_cmd); i++)
		{
			Uart_PutChar(currnet_uart,current_sensor_cmd[i]);
			current_sensor_index = 0;
		}
	}
...
#else
	Uart_Configuration (&Uart1, USART1, 4800, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No);
	current_sensor_init(&Uart1);
#endif
