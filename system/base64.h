...
{
    std::string x(base64_encode_len(s.size()), '\0');
    int d = base64_encode(const_cast<char*>(x.data()), s.data(), s.size());
...
{
    std::string x(base64_decode_len(s.size()), '\0');
    int d = base64_decode(const_cast<char*>(x.data()), s.data(), s.size());
    if (d < 0) {
        x.clear();
    } else {
        x.erase(d, std::string::npos);
