...
void HID_MOUSE_UpdatePosition (int8_t x, int8_t y)
{
  ...
  if ((x != 0) || (y != 0)) 
  {
    ...
    if(y_loc > MOUSE_WINDOW_HEIGHT - 15 ){  //- 12
      y_loc = MOUSE_WINDOW_HEIGHT - 15; //- 12
    }  
    if(x_loc > MOUSE_WINDOW_WIDTH  - 15){  //- 10
      x_loc = MOUSE_WINDOW_WIDTH  - 15;  //- 10
    }  
    if(y_loc < 0){
      y_loc = 0;
    }  
    ...
    {
      LCD_DisplayChar(MOUSE_WINDOW_X - 8 + prev_y ,\
        MOUSE_WINDOW_Y + 15 + prev_x, ' ');
    }
    else if((prev_x == 0) && ( prev_y == 0))
    {
      
      LCD_DisplayChar(MOUSE_WINDOW_X - 8 + 1,
                                MOUSE_WINDOW_Y + 15 + 1,
                                ' ');    
    }
    
    LCD_DisplayChar  (MOUSE_WINDOW_X - 8 + y_loc ,\
      MOUSE_WINDOW_Y + 15 + x_loc, 'x');
