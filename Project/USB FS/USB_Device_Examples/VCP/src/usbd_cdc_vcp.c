...
static uint16_t VCP_Init(void)
{
  ...
  /* EVAL_COM3 default configuration */
  /* EVAL_COM3 configured as follow:
...
static uint16_t VCP_DataTx (uint8_t* Buf, uint32_t Len)
{
  ...
  {
    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(EVAL_COM3) & 0x7F;
  }
  else if (linecoding.datatype == 8)
  {
    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(EVAL_COM3);
  }
...
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  ...
  for (i = 0; i < Len; i++)
  {
    USART_SendData(EVAL_COM3, *(Buf + i) );
    while(USART_GetFlagStatus(EVAL_COM3, USART_FLAG_TXE) == RESET); 
  } 
