...
//		local_para_conf[6] = 0;
...
#if RADIO_TYPE == XTEND915
			GUI_SetColor(GUI_DARKBLUE);	

#elif RADIO_TYPE == JIZUO433			
		...
		GUI_SetColor(GUI_GRAY);	
		GUI_DispStringAt("0", 128, 17 + 25);
		GUI_DispStringAt("0", 128, 17 + 50);
		GUI_DispStringAt("0", 128, 17 + 75);
...
#elif RADIO_TYPE == JIZUO433			
		GUI_SetColor(GUI_GRAY);	
		float_to_string(0, tempChar, 0, 0, 0, 0);				//地址掩码
		GUI_DispStringAt(tempChar, 128, 17);
		GUI_DispStringAt("500 mW", 128, 42);						//电台发射功率
		float_to_string(0, tempChar, 0, 0, 0, 0);				//API模式
		GUI_DispStringAt(tempChar, 128, 67);	
		
		GUI_SetTextAlign(GUI_TA_RIGHT | GUI_TA_VCENTER);
		float_to_string(0, tempChar, 0, 0, 0, 0);				//电台温度
		GUI_DispStringAt(tempChar, 156, 92);
...
#elif RADIO_TYPE == JIZUO433			
		GUI_SetColor(GUI_RED);
		float_to_string(Xtend_900_para_buf[0], tempChar, 0, 0, 0, 0);
		GUI_DispStringAt(tempChar, 128, 17);
...
//					temp_num = RightRudderCommand;
					temp_num = Right_Joystick_X_axis_Avg;

					//ME300舵角
					if(local_para_conf[2] == USV_Boat_Type_ME300)
					{
						if(temp_num >= MAX_RUDDER_SHOW)
						{
							temp_num = MAX_RUDDER_SHOW;
						}
						else if(temp_num <= MIN_RUDDER_SHOW)
						{
							temp_num = MIN_RUDDER_SHOW;
						}
						
						if((temp_num >= DEFAULT_MIDDLE_VALUE + RUDDER_ZERO_RANGE) && (temp_num <= MAX_RUDDER_SHOW))
						{
							temp_num = temp_num - DEFAULT_MIDDLE_VALUE;
							GUI_DispStringAt(_xiangzuo[0], 214, 70);												//转左		
							
							float_to_string(temp_num * 100 / (MAX_RUDDER_SHOW - DEFAULT_MIDDLE_VALUE), tempChar, 0, 1, 0, 0);				//舵角百分比			
						}
						else if((temp_num <= DEFAULT_MIDDLE_VALUE - RUDDER_ZERO_RANGE) && (temp_num >= MIN_RUDDER_SHOW))
						{
							temp_num = DEFAULT_MIDDLE_VALUE - temp_num;
							GUI_DispStringAt(_xiangyou[0], 214, 70);												//转右			
							
							float_to_string(temp_num * 100 / (DEFAULT_MIDDLE_VALUE - MIN_RUDDER_SHOW), tempChar, 0, 1, 0, 0);				//舵角百分比	
						}
						else
						{
							temp_num = 0;
			...
			default:
			{
				if(local_para_conf[6] == 1)																		//中文显示
				{
					...
					else														
					{
						...
						temp_num = Left_Joystick_X_axis_Avg;
