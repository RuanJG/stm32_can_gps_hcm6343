...
//		USART_SendData(USART2, 0x89);
		...
		if(K_div > 1000)
		{
			...
			//参数配置时修改索引修改---------------------------------------------------------------------------------			
			//索引减操作，即左摇杆往上拨
			if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (left_joystick_ud == 1))
			{
				//判断索引是否到顶，不到顶减一处理
				if(modify_index > 0)
				{
			...
			//索引加操作，即左摇杆往下拨
			else if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (left_joystick_ud == 2))
			{
				//判断索引是否到底，不到底加一处理
				//ME300存在若干控制阀门----------------------------------------------
...
//					if(modify_index < PARAMETER_QUANTITY + LOCAL_PARAMETER_QUANTITY + VALVE_QUANTITY - 1)
			...
			//索引换列操作，即左摇杆往左拨或右拨
			if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && ((left_joystick_lr == 1) || (left_joystick_lr == 2)))
			{
			...
			//采样配置时修改索引修改---------------------------------------------------------------------------------			
			//索引减操作，即左摇杆往上拨
			if(((interface_index == DEBUG_SAMPLE_INTERFACE_INDEX) || (interface_index == MAIN_SAMPLE_INDEX)) && (left_joystick_ud == 1))
			{
				//判断索引是否到顶，不到顶减一处理
				if(modify_index > 0)
				{
			...
			//索引加操作，即左摇杆往下拨
			else if(((interface_index == DEBUG_SAMPLE_INTERFACE_INDEX) || (interface_index == MAIN_SAMPLE_INDEX)) && (left_joystick_ud == 2))
			{
				//判断索引是否到底，不到底加一处理
			...
			//当前索引下的参数慢速修改（参数配置）-------------------------------------------------------------------------------
			//参数加操作，即右摇杆往上拨
			if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (right_joystick_ud == 1))
			{
			...
			//参数减操作，即右摇杆往下拨
			else if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (right_joystick_ud == 2))
			{
				// 0 ~ PARAMETER_QUANTITY - 1 索引时
				//数传模块参数索引-----------------------------------------------------------------------
				if(modify_index < PARAMETER_QUANTITY)
			...
			//当前索引下的参数慢速修改（采样配置）-------------------------------------------------------------------------------
			//参数加操作，即右摇杆往上拨
			if(((interface_index == DEBUG_SAMPLE_INTERFACE_INDEX) || (interface_index == MAIN_SAMPLE_INDEX)) && (right_joystick_ud == 1))
			{
				//判断当前参数是否到顶，到顶后重新赋值至底部
			...
			//参数减操作，即右摇杆往下拨
			else if(((interface_index == DEBUG_SAMPLE_INTERFACE_INDEX) || (interface_index == MAIN_SAMPLE_INDEX)) && (right_joystick_ud == 2))
			{
				//判断当前参数是否到底，到底后重新赋值至顶部
		...
		if(KQ_div >= 2)
		{
			...
			//当前索引下的参数快速修改-------------------------------------------------------------------------------
			//参数加操作，即右摇杆往左拨
			if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (right_joystick_lr == 1))
			{
				// 0 ~ PARAMETER_QUANTITY - 1 索引时
				if(modify_index < PARAMETER_QUANTITY)
				{
			...
			//参数减操作，即右摇杆往右拨
			else if(((interface_index == DEBUG_PARAM_INTERFACE_INDEX) || (interface_index == MAIN_PARAM1_INDEX)) && (right_joystick_lr == 2))
			{
				// 0 ~ PARAMETER_QUANTITY - 1 索引时
				if(modify_index < PARAMETER_QUANTITY)
				{
