...
void main_sample_show(void)
{
	...
	if(debug_mode_parameter_init == 1)
	{
		...
		{
			GUI_DispStringAt("Sampling Configuration", 54, 18);					
		}
...
//		
//		//刷新LCD区域
//		GUI_MEMDEV_CopyToLCDAt(bg0_memdev, DEBUG_BG0_XAXIS, 50);				
	}
...
//		local_para_conf[6] = 0;
		...
		GUI_SetBkColor(GUI_WHITE);
		GUI_SetFont(&GUI_Font16_1);
		GUI_DispStringAt(version_number, 180, 18);	
...
//		
//		GUI_MEMDEV_CopyToLCDAt(bg1w_memdev, 180, 0);
...
//		local_para_conf[6] = 1;
	...
	if(MODE_Button == CONTROL_MODE_MANUAL)			//单摇杆模式
		GUI_BMP_Draw(_acImage_5, 9, 8);	
	
	else if(MODE_Button == CONTROL_MODE_MANUAL2)
	{
		GUI_BMP_Draw(_acImage_5, 0, 8);	
		GUI_BMP_Draw(_acImage_5, 18, 8);
	}
	else 
	{
		GUI_SetFont(&GUI_Fontsongti16);
...
//		local_para_conf[6] = 1;
		...
		if(local_para_conf[6] == 1)		//中文
			GUI_DispStringAt(_zidong[0], 3, 18);	
		
		else													//英文
			GUI_DispStringAt("Auto", 3, 18);
	}	
...
//	}
	...
	if(ALARM_Button == 1)
	{
		GUI_BMP_Draw(_acImage_9, 85, 3);				//警报灯图标
	}
	else
	{
		GUI_SetColor(GUI_TRANSPARENT);
		GUI_FillRect(85, 3, 29 + 85, 33);	
	}
	...
	GUI_SetTextAlign(GUI_TA_CENTER | GUI_TA_VCENTER);
	GUI_DispStringAt(tempChar, 60, 18);	
	
	GUI_MEMDEV_CopyToLCDAt(bg1w_memdev, 56, 0);
	...
	if((BoatDataTimeoutCount <= 5) && (connect_ok == 1))						
	{
		GUI_BMP_Draw(_acImage_6, 3, 3);				//连接成功显示
	}
	else
	{
		GUI_BMP_Draw(_acImage_7, 3, 3);				//连接失败显示
	}
	GUI_MEMDEV_CopyToLCDAt(bg0s_memdev, 96, 0);
...
//	GUI_MEMDEV_CopyToLCDAt(bg1w_memdev, 96, 0);
...
//	BoatData.Speed = 233;
	...
	GUI_SetTextAlign(GUI_TA_RIGHT | GUI_TA_VCENTER);
	float_to_string(((double) BoatData.Speed) / 100, tempChar, 2, 0, 0, 0);
...
//	local_para_conf[6] = 0;
	...
	if(local_para_conf[6] == 1)					//中文
	{
		GUI_DispStringAt(_souyao[0], 129, 52);									//艘摇 
	...
	GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
	float_to_string(BoatData.Yaw, tempChar, 0, 0, 0, 0);
	...
	GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
	float_to_string(BoatData.Roll, tempChar, 0, 0, 0, 0);
...
//	BoatData.Pitch = 12048;
	...
	GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
	float_to_string(BoatData.Pitch, tempChar, 0, 0, 0, 0);
...
//	BoatData.WaterTemp = -32.5 / 10;
	...
	GUI_SetTextAlign(GUI_TA_RIGHT | GUI_TA_VCENTER);
	float_to_string(BoatData.WaterTemp * 10, tempChar, 1, 0, 0, 0);
...
//	BoatData.InternalTemp = 132.5 / 10;
	...
	GUI_SetTextAlign(GUI_TA_RIGHT | GUI_TA_VCENTER);
	float_to_string(BoatData.InternalTemp * 10, tempChar, 1, 0, 0, 0);
...
//BoatData.Depth = 60.3 / 100;
	...
	if(show_depth_div >= SHOW_DEPTH_TIME)					//0.5秒刷新一次
	{
		...
		GUI_MEMDEV_Clear(bg1_memdev);	
		GUI_SetColor(GUI_TRANSPARENT);
		GUI_FillRect(30 + waveform_xaxis + 1, 0, 30 + waveform_xaxis + 1, DEBUG_BG1_YAXIS);		
	
		//读取当前深度后转换为显示输出
		temp_num = (uint16_t) (BoatData.Depth * 100);
		
		if(temp_num >= SHOW_MAX_DEPTH)
		{
			temp_num = SHOW_MAX_DEPTH;
		}
		
		//绘画当前位置深度直线
		GUI_SetColor(GUI_RED);
		GUI_DrawLine(30 + waveform_xaxis, temp_num, 30 + waveform_xaxis + 1, temp_num);
