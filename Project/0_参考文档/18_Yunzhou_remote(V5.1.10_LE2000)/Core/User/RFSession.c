...
void GenerateTakeSampleCommandBase64(bool encoded)
{
	...
	if(NeedSendSampleCount > 0){
		...
		if(encoded == TRUE)
		{
			...
			//计算API传输的checksum
			for(tempbyte = 0; tempbyte < sendBuffer_encoded_length; tempbyte++)
			{
				api_checksum += sendBuffer_encoded[tempbyte];
			}		
			
			api_checksum += 0x01;
			api_checksum += 0x52;
			api_checksum += (uint8_t) (Xtend_900_para[3] & 0x00ff);
			api_checksum += (uint8_t) ((Xtend_900_para[3] & 0xff00) >> 8);
			api_checksum += 0x00;
			api_checksum += 0xFA;
			api_checksum += 0xFA;
			api_checksum += 0xFE;
			api_checksum += 0xFE;
			api_checksum = 0xff - api_checksum;					

			if(Xtend_900_para[6] == 0)			//915，透传模式
			{
				PutUART(0xFA);
				PutUART(0xFA);
			...
			else if(Xtend_900_para[6] == 1)			//915，API模式 no escape
			{
				PutUART(0x7E);					//Start Delimiter
				PutUART(0x00);					//Length, 高8位
				PutUART(sendBuffer_encoded_length + 9);			//API长度，低8位
				PutUART(0x01);					//发送命令
				PutUART(0x52);					//"R" value was arbitrarily selected
				PutUART((uint8_t) (Xtend_900_para[3] & 0x00ff));				//目的地址低8位
				PutUART((uint8_t) ((Xtend_900_para[3] & 0xff00) >> 8));				//目的地址高8位
				PutUART(0x00);					//Standard
				PutUART(0xFA);
				PutUART(0xFA);			
				transmit_data(sendBuffer_encoded, sendBuffer_encoded_length);					//数据包
				PutUART(0xFE);
				PutUART(0xFE);	
				PutUART(api_checksum);						//API传输报的校验值					
			}			
		}
		else {
...
void SendSampleFinished(bool encoded)
{
	...
	if(encoded == TRUE)
	{
		...
		//计算API传输的checksum
		for(tempbyte = 0; tempbyte < modBufferEncodedLength; tempbyte++)
		{
			api_checksum += modBufferEncoded[tempbyte];
		}			
		...
		else if(Xtend_900_para[6] == 1)			//915，API模式 no escape
		{
			PutUART(0x7E);					//Start Delimiter
			PutUART(0x00);					//Length, 高8位
			PutUART(modBufferEncodedLength + 9);			//API长度，低8位
			PutUART(0x01);					//发送命令
			PutUART(0x52);					//"R" value was arbitrarily selected
			PutUART((uint8_t) (Xtend_900_para[3] & 0x00ff));				//目的地址低8位
			PutUART((uint8_t) ((Xtend_900_para[3] & 0xff00) >> 8));				//目的地址高8位
			...
			PutUART(0xFA);
			PutUART(0xFA);			
			transmit_data(modBufferEncoded, modBufferEncodedLength);					//数据包
			PutUART(0xFE);
			PutUART(0xFE);	
			PutUART(api_checksum);						//API传输报的校验值					
		}
...
//			checksum += tempbyte;
	...
	if(encoded == TRUE)
	{
		...
		if(Xtend_900_para[6] == 0)					//151008_数据透传模式
		{
			PutUART(0xFA);
			PutUART(0xFA);
			transmit_data(sendBuffer_encoded, sendBuffer_encoded_length);
...
//	      result.USVDateData.Day = bytearray[startindex + 3];
	   ...
	   case 6:      //remaining battery 1 byte
		 ...
		 case 8:														//151008: MissionID
				startindex += 2 + 1;
         break;

		 case 9:														//151008: Interval
				startindex += 2 + 1;
         break;		 
...
//							LCD_WR_L_HanZi_String(91, 120 , (u8*)hao ping );
        ...
        break;
			...
			case 92:			//151008: 水流方向
			{
				if (startindex + 4 > endindex) //防止一条消息只收了一部分
					return;
				//result.WaterParaData.Depth = (float)Math.Round(BitConverter.ToSingle(bytearray, startindex + 1), 2);
				flow_direction = BytesToFloat(&(bytearray[startindex + 1]));
				startindex += 4 + 1;
				//if (startindex >= endindex)
				//	return;
				break;				
			...
			case 93:       //151008: 水速
			{
				if (startindex + 4 > endindex) //防止一条消息只收了一部分
					return;
				//result.WaterParaData.Depth = (float)Math.Round(BitConverter.ToSingle(bytearray, startindex + 1), 2);
				flow_speed = BytesToFloat(&(bytearray[startindex + 1]));
				startindex += 4 + 1;
				//if (startindex >= endindex)
				//	return;
				break;	
			...
			case 94:       //151008: ORP 氧化还原电位
			{
				if (startindex + 4 > endindex) //防止一条消息只收了一部分
					return;
				//result.WaterParaData.Depth = (float)Math.Round(BitConverter.ToSingle(bytearray, startindex + 1), 2);
				ORP_num = BytesToFloat(&(bytearray[startindex + 1]));
				startindex += 4 + 1;
				//if (startindex >= endindex)
				//	return;
				break;	
			...
			case 95:       //151008: TDS 固体溶解度
			{
				if (startindex + 4 > endindex) //防止一条消息只收了一部分
					return;
				//result.WaterParaData.Depth = (float)Math.Round(BitConverter.ToSingle(bytearray, startindex + 1), 2);
				TDS_num = BytesToFloat(&(bytearray[startindex + 1]));
				startindex += 4 + 1;
				//if (startindex >= endindex)
				//	return;
				break;	
			...
			case 96:       //151008: NH4+ 铵离子
			{
				if (startindex + 4 > endindex) //防止一条消息只收了一部分
					return;
				//result.WaterParaData.Depth = (float)Math.Round(BitConverter.ToSingle(bytearray, startindex + 1), 2);
				NH4_num = BytesToFloat(&(bytearray[startindex + 1]));
				startindex += 4 + 1;
				//if (startindex >= endindex)
				//	return;
				break;	
	...
	BoatData.flow_direction = flow_direction;
	BoatData.flow_speed = flow_speed;
	BoatData.ORP_num = ORP_num;
	BoatData.TDS_num = TDS_num;
	BoatData.NH4_num = NH4_num;	
}
