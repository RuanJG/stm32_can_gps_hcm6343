...
void main_param_show(void)
{
	...
	if(debug_change_flag == 1)
	{
		...
		{
			GUI_DispStringAt(_diantaishezhi[0], 54, 18);								//电台设置
		}	
		else
		{
			GUI_DispStringAt("RF Configuration", 54, 18);					
		}
		...
		{
			GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
			GUI_DispStringAt(_tongxinpindao[0], 28, 18);								//通信频道
		...
		GUI_MEMDEV_CopyToLCDAt(bg0_memdev, 0, 62);	
