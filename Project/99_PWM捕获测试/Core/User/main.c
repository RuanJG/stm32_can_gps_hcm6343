...
{   
  ...
  {			
		...
		GUI_DispStringAt("             ---for 900M", 30, 35);
		...
		GUI_DispStringAt("the Frequency is:", 30, 70);
		GUI_DispStringAt(tempChar, 130, 70);
		
		rdbm = DutyCycle * 2 / 3 - 113;		
		float_to_string(rdbm, tempChar, 0, 3, 0, 0);				
		GUI_DispStringAt("The RSSI is:", 30, 81);
		GUI_DispStringAt(tempChar, 100, 81);
		
		float_to_string(receive_counter, tempChar, 0, 0, 0, 0);				
		GUI_DispStringAt("packages no:", 30, 92);
		GUI_DispStringAt(tempChar, 120, 92);
