...
修改说明：<<-------------------------------------------------------->>
					日期   修改人    版本号		修改内容
					150603 趙铭章    5.0.0		新建立
																
					<<-------------------------------------------------------->>
					日期   修改人    版本号		修改内容
					150910	趙铭章	 5.1.5		南方测绘显示
...
//									USART_SendData(USART2, 0x68);
				...
				if(RF_transmit_timer >= (RF_TRANSMIT_TIMES - RF_TRANSMIT_TIMES_TOLER - RF_transmit_timer_toler))					
				{
					...
					switch(QC_Test_Flag)
					{
						...
						case 9:
						case 10:
							if(QC_Test_Timer >= QC_TEST_TIME - 90) 			//30秒切换一次动作
							{
								QC_Test_Timer = 0;
								...
								if(QC_Test_Flag > QC_TEST_STATUS)			//循环测试
								{
									QC_Test_Flag = 1;
								}
							}
						...
						case 7:
						case 8:
							if(QC_Test_Timer >= QC_TEST_TIME + 30) 			//50秒切换一次动作
							{
								QC_Test_Timer = 0;
					...
					if(QC_Motor_Timer >= QC_TEST_TIME - 90)
					{
						QC_Motor_Timer = 0;
...
//合作企业为南方测绘
#elif (GPS_COOPERATE == SOUTH_SATELLITE_RTK)
		//绘制标题
		GUI_SetColor(GUI_DARKBLUE);
		GUI_SetBkColor(GUI_WHITE);
		GUI_SetFont(&GUI_Fontsongti16);
		...
		if(local_para_conf[6] == 1)						//中文
		{
			GUI_DispStringAt(_lianxifangshi[0], 18, 18);								//联系方式
		}	
		else
		{
			GUI_DispStringAt("Contact Us", 18, 18);					
		}		
#endif		
...
//	GUI_BMP_Draw(_acImage_24, 36, 8);											//南方测绘 logo
...
//		
//		GUI_SetColor(GUI_BLUE);
//		GUI_DispStringAt(_lianxidizhi[0], 8, 88);												//联系地址
//		GUI_DispStringAt(":", 73, 88);
//	}		
//	
//	//刷新LCD区域
//	GUI_MEMDEV_CopyToLCDAt(bg0_memdev, 0, 36);	
...
//		
//		GUI_SetColor(GUI_RED);		
//		GUI_DispStringAt(_zhongguoguangdongshengguangzhoushikeyunlu[0], 36, 10);					//中国广东省广州市科韵路
//		GUI_DispStringAt(_ruanjianyuanjianzhonglu52haodaohangdasha[0], 8, 36);						//软件园建中路52号导航大厦
//		
//		GUI_SetColor(GUI_BLUE);
//		GUI_DispStringAt(_dianhuahaoma[0], 8, 62);										//电话号码
//		GUI_DispStringAt(":", 73, 62);
//		GUI_SetColor(GUI_RED);
//		GUI_SetFont(&GUI_Font16_1);
//		GUI_DispStringAt("400-7000-700", 86, 62);

//		GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
//		GUI_SetFont(&GUI_Fontsongti16);		
//		GUI_SetColor(GUI_BLUE);
//		GUI_DispStringAt(_guanfangwangzhi[0], 8, 88);										//官方网址
//		GUI_DispStringAt(":", 73, 88);
//		GUI_SetColor(GUI_RED);
//		GUI_SetFont(&GUI_Font16_1);
//		GUI_DispStringAt("www.southgnss.com", 86, 88);		
//	}	
//	else
//	{
////		GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
...
//	//刷新LCD区域
//	GUI_MEMDEV_CopyToLCDAt(bg0_memdev, 0, 136);	
...
//	GUI_BMP_Draw(_acImage_25, 36, 18);											//云洲 logo
...
//	
//	//刷新LCD区域
//	GUI_MEMDEV_CopyToLCDAt(bg0_memdev, DEBUG_BG0_XAXIS, 36);	
...
//		
//		GUI_SetColor(GUI_RED);		
//		GUI_DispStringAt(_zhongguoguangdongshengzhuhaishi[0], 36, 10);								//中国广东省珠海市高新区
//		GUI_DispStringAt(_nanfangruanjianyuan[0], 8, 36);															//南方软件园D2-214
//		
//		GUI_SetColor(GUI_BLUE);
//		GUI_DispStringAt(_dianhuahaoma[0], 8, 62);										//电话号码
//		GUI_DispStringAt(":", 73, 62);
//		GUI_SetColor(GUI_RED);
//		GUI_SetFont(&GUI_Font16_1);
//		GUI_DispStringAt("+86(756)3626636", 86, 62);
...
//		GUI_SetColor(GUI_RED);
//		GUI_SetFont(&GUI_Font16_1);
//		GUI_DispStringAt("www.yunzhou-tech.com", 86, 88);		
//	}	
//	else
//	{
////		GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
...
//	//刷新LCD区域
//	GUI_MEMDEV_CopyToLCDAt(bg0_memdev, DEBUG_BG0_XAXIS, 136);		
#endif	
...
//合作企业为南方测绘
		...
		if(local_para_conf[6] == 1)						//中文
		{
			GUI_DispStringAt(_canshushezhi[0], 8, 18);								//参数设置
		}	
		else
		{
			GUI_DispStringAt("USV Configure", 3, 18);					
		}	
#endif	
...
//合作企业为无
#if (GPS_COOPERATE == HOME)
		GUI_DispStringAt(version_number, 180 - 66, 18);								//显示版本号
#endif
...
//合作企业为南方测绘		
#if (GPS_COOPERATE == SOUTH_SATELLITE_RTK)
		//绘制过渡窗口
		GUI_DrawGradientV (0, 0, DEBUG_BG0_XAXIS, 3, GUI_BLACK, GUI_WHITE);		
		GUI_SetColor(GUI_WHITE);
		GUI_FillRect(0, 3, DEBUG_BG0_XAXIS, 33);
		...
		GUI_BMP_Draw(_acImage_23, 0, 3);					//South logo 南方测绘logo		

		GUI_SetColor(GUI_DARKBLUE);
		GUI_SetBkColor(GUI_WHITE);
		GUI_SetFont(&GUI_Font16_1);
#if defined QC_LOOP_TEST	
		GUI_DispStringAt(version_number, 200, 18);								//显示版本号
#else
		GUI_DispStringAt(version_number, 210, 18);								//显示版本号
#endif

		GUI_MEMDEV_CopyToLCDAt(bg0_memdev, 176 - 10, 0);		
#endif		
...
//合作企业为南方测绘		
		...
		switch (local_para_conf[2])
		{
			...
			case USV_Boat_Type_SSV40A1:									//方舟号
			{
				//绘制深度显示背景
				GUI_SelectLayer(0);
				GUI_MEMDEV_Select(bg0_memdev);	
...
//合作企业为南方测绘
		...
		GUI_DrawGradientV(30, 10 + 10, 36, 25 + 10, GUI_DARKRED, GUI_LIGHTRED);				//绘画伪彩深度映射
		GUI_DrawGradientV(30, 25 + 10, 36, 40 + 10, GUI_LIGHTRED, GUI_YELLOW);
		GUI_DrawGradientV(30, 40 + 10, 36, 55 + 10, GUI_YELLOW, GUI_GREEN);
		GUI_DrawGradientV(30, 55 + 10, 36, 70 + 10, GUI_GREEN, GUI_LIGHTBLUE);
		GUI_DrawGradientV(30, 70 + 10, 36, 85 + 10, GUI_LIGHTBLUE, GUI_DARKBLUE);
		...
		for(temp_num = 0; temp_num < 8; temp_num++)
		{
			GUI_SetTextAlign(GUI_TA_RIGHT | GUI_TA_VCENTER);
			float_to_string(temp_num * 10, tempChar, 1, 0, 0, 0);
			GUI_DispStringAt(tempChar, 26, 20 + temp_num * 10);					
		}
		
		//绘画经纬度辅助线		
		GUI_SetLineStyle(GUI_LS_DOT);					//切换成虚线
		GUI_SetColor(GUI_GRAY);
		
		//纬度辅助线
		for(temp_num = 0; temp_num < 8; temp_num++)
		{		
			GUI_DrawLine(48, 18 + temp_num * 10, 214, 18 + temp_num * 10);		
		}
		
		//经度辅助线
		for(temp_num = 1; temp_num <= 16; temp_num++)
		{		
			GUI_DrawLine(48 + temp_num * 10, 10, 48 + temp_num * 10, 96);		
		}		
...
//				printf("RRS: %d, %d,  LRS: %d, %d\n", RightRudderCommand, RightSpeedCommand, LeftRudderCommand, LeftSpeedCommand);
	...
	{
		case 1:		
			GUI_DispStringAt("L & A", 170, 30);
			break;
		
		case 2:	
			GUI_DispStringAt("R & B", 170, 30);
			break;

		case 3:	
			GUI_DispStringAt("R & A", 170, 30);
			break;	

		case 4:	
			GUI_DispStringAt("L & B", 170, 30);			
			break;

		case 5:		
			GUI_DispStringAt("S 1    ", 170, 30);
			break;	

		case 6:		
			GUI_DispStringAt("S 2    ", 170, 30);
			break;

		case 7:		
			GUI_DispStringAt("S 3    ", 170, 30);
			break;		

		case 8:		
			GUI_DispStringAt("S 4    ", 170, 30);
			break;		

		case 9:		
			GUI_DispStringAt("LED    ", 170, 30);
			break;

		case 10:			
			GUI_DispStringAt("Alarm", 170, 30);		
			break;
	...
	switch(QC_Test_Flag)
	{
		...
		case 9:
		case 10:	
			float_to_string(QC_TEST_TIME - QC_Test_Timer - 90, tempChar, 0, 0, 0, 0);				//显示测试进度
			break;
		...
		case 7:
		case 8:	
			float_to_string(QC_TEST_TIME - QC_Test_Timer + 30, tempChar, 0, 0, 0, 0);				//显示测试进度
			break;				
	}
	
	GUI_DispStringAt(tempChar, 210, 30);
	//质检测试用显示当前动作及时间==========================
#endif
...
//		GUI_DispStringAt(_caiyangyujiance[0], 16, 12);										//采样 & 监测		
			GUI_DispStringAt(_shishijiance[0], 16, 12);										//实时监测	
	}	
	else
	{
		GUI_SetFont(&GUI_Fontsongti12);
	...
	GUI_DrawGradientRoundedH(8, 2, 116, 22, 3, GUI_RED, GUI_ORANGE);				//实时监测背景条
