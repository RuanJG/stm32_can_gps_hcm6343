...
uint32_t Xtend_900_para_min[PARAMETER_QUANTITY] = {1, 0, 0, 0, 0, 0, 0, 0};														//定义最小值
uint32_t Xtend_900_para_max[PARAMETER_QUANTITY] = {16, 0, 0, 0, 0, 0, 0, 0};													//定义最大值	
...
//	USART1_TX_Buffer[56] = 0x20;
//	USART1_TX_Buffer[57] = 0x0d;
...
//	printf("%s", USART1_TX_Buffer);
	...
	for(i = 0; i < 30; i++)
	{
		...
		PutUART((uint8_t) Xtend_900_para[0]);
		PutUART(0); //C
		PutUART(149 + (uint8_t) Xtend_900_para[0]);
		PutUART(13);
		PutUART(10);	
	}
...
/**
  ...
  * @retval 返回显示字符串
  */
char * get_TX_Power(uint16_t power_select)
{
	switch(power_select)
	{
		case 0:
